...
/*
    "TAB" - nusoka per simbolius per 4 kartotini

    'string'.charAt(0) - elgiasi kaip masyvas, t.y. kaip 'Petras'[0];
    'string'.at(0) - universalesnis budas, gali nuo galo

    'string'.inludes(0) - ar ieskomas dalykas yra ieskomame dalyke, kalba yra apie is eiles einancius simbolius
    'string'.startsWith(0) - panasus kaip include, tik nuo pradzios ima
    'string'.endsWith(0) - panasus kaip include, tik nuo pabaigos ima

    'string'.toLowerCase() - teksto raides koncertuoja i mazasias
    'string'.toUpperCase() - teksto raides koncertuoja i didziasias

    'string'.indexOf() - kur prasideda dominantis dalykas

    'string'.slice() - ima nurodyta simboliu kieki nuo pasirinkto starto iki pasirinktos pabaigos
    'string'.slice(1) - jei be pirmo ir be paskutinio
    'string'.slice(2, -2) - nuimk pirmus du ir paskutinius du
    'string'.slice(-2) - paima paskutinius 2

    'string'.replace()
    'string'.replaceAll() - pakeisti visus besikartojancius. Gali nevekti, jei node versija sena, isprendziama su regex

    regex / regexp - iesko sablonu, vengti, nes sablonai gali buti ziauriai keisti :))
    Labai patogu hakeriams isilauzti, kai nesupranti regex veikimo ir parasai koda su spragom. 
    Regex minusas - nepasako, kur klaida
